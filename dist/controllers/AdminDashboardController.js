    ...
    static async getAdminDashboardSummary(req, res) {
        try {
            ...
            const [totalUsers, verifiedUsers, activeUsers, newUsersToday, newUsersThisWeek, newUsersThisMonth, newUsersLastMonth] = await Promise.all([
                ...
                userRepo.count({
                    where: { date_joined: (0, typeorm_1.Between)(today, now) }
                }),
                userRepo.count({
                    where: { date_joined: (0, typeorm_1.Between)(thisWeekStart, now) }
                }),
                userRepo.count({
                    where: { date_joined: (0, typeorm_1.Between)(thisMonthStart, now) }
                }),
                userRepo.count({
                    where: { date_joined: (0, typeorm_1.Between)(lastMonthStart, lastMonthEnd) }
                })
            ]);
            ...
            const [totalProjects, publishedProjects, draftProjects, archivedProjects, newProjectsToday, newProjectsThisWeek, newProjectsThisMonth] = await Promise.all([
                ...
                projectRepo.count({
                    where: { created_at: (0, typeorm_1.Between)(today, now) }
                }),
                projectRepo.count({
                    where: { created_at: (0, typeorm_1.Between)(thisWeekStart, now) }
                }),
                projectRepo.count({
                    where: { created_at: (0, typeorm_1.Between)(thisMonthStart, now) }
                })
            ]);
            ...
            const pendingJoinRequests = await joinRequestRepo.count({
                where: { status: CommunityJoinRequest_1.JoinRequestStatus.PENDING }
            });
            ...
            const [totalEvents, upcomingEvents, ongoingEvents, completedEvents, cancelledEvents, newEventsToday, newEventsThisWeek, newEventsThisMonth] = await Promise.all([
                eventRepo.count({
                ...
                eventRepo.count({
                    where: {
                        ...
                        start_datetime: (0, typeorm_1.Between)(now, new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000))
                    }
                }),
                eventRepo.count({ where: { status: Event_1.EventStatus.ONGOING } }),
                eventRepo.count({ where: { status: Event_1.EventStatus.COMPLETED } }),
                eventRepo.count({ where: { status: Event_1.EventStatus.CANCELLED } }),
                eventRepo.count({
                    where: { created_at: (0, typeorm_1.Between)(today, now) }
                }),
                eventRepo.count({
            ...
            const [totalRegistrations, approvedRegistrations, attendedRegistrations] = await Promise.all([
                attendeeRepo.count(),
                attendeeRepo.count({ where: { registration_status: EventAttendee_1.RegistrationStatus.APPROVED } }),
                attendeeRepo.count({ where: { registration_status: EventAttendee_1.RegistrationStatus.ATTENDED } })
            ]);
            // Upcoming events (next 7 days)
            const upcomingEventsList = await eventRepo.find({
                where: {
                    status: Event_1.EventStatus.UPCOMING,
                    start_datetime: (0, typeorm_1.Between)(now, new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000))
                },
                relations: ["organizer"],
            ...
            const [totalBlogs, publishedBlogs, totalQAThreads, answeredQAThreads, totalCommunityPosts] = await Promise.all([
                blogRepo.count(),
                blogRepo.count({ where: { status: ResearchProject_1.ProjectStatus.PUBLISHED } }),
