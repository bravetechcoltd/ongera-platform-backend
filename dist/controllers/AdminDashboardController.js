...
class AdminDashboardController {
    ...
    static async getAdminDashboardSummary(req, res) {
        try {
            ...
            const [totalUsers, verifiedUsers, activeUsers, newUsersToday, newUsersThisWeek, newUsersThisMonth, newUsersLastMonth] = await Promise.all([
                userRepo.count(),
                userRepo.count({ where: { is_verified: true } }),
                userRepo.count({ where: { is_active: true } }),
                userRepo.count({ where: { date_joined: (0, typeorm_1.Between)(today, now) } }),
                userRepo.count({ where: { date_joined: (0, typeorm_1.Between)(thisWeekStart, now) } }),
                userRepo.count({ where: { date_joined: (0, typeorm_1.Between)(thisMonthStart, now) } }),
                userRepo.count({ where: { date_joined: (0, typeorm_1.Between)(lastMonthStart, lastMonthEnd) } })
            ]);
            ...
            const [totalJoinRequests, pendingJoinRequests, approvedJoinRequests, rejectedJoinRequests] = await Promise.all([
                joinRequestRepo.count(),
                joinRequestRepo.count({ where: { status: CommunityJoinRequest_1.JoinRequestStatus.PENDING } }),
                joinRequestRepo.count({ where: { status: CommunityJoinRequest_1.JoinRequestStatus.APPROVED } }),
                joinRequestRepo.count({ where: { status: CommunityJoinRequest_1.JoinRequestStatus.REJECTED } })
            ]);
            ...
            const [totalCommunities, activeCommunities, pendingCommunities, newCommunitiesToday, newCommunitiesThisWeek, newCommunitiesThisMonth] = await Promise.all([
                communityRepo.count(),
                communityRepo.count({ where: { is_active: true } }),
                communityRepo.count({ where: { is_active: false } }),
                communityRepo.count({ where: { created_at: (0, typeorm_1.Between)(today, now) } }),
                communityRepo.count({ where: { created_at: (0, typeorm_1.Between)(thisWeekStart, now) } }),
                communityRepo.count({ where: { created_at: (0, typeorm_1.Between)(thisMonthStart, now) } })
            ]);
            const communityEngagement = await communityRepo
                ...
                .getRawOne();
            // Project basic stats
            const [totalProjects, publishedProjects, draftProjects, archivedProjects, newProjectsToday, newProjectsThisWeek, newProjectsThisMonth] = await Promise.all([
                projectRepo.count(),
                projectRepo.count({ where: { status: ResearchProject_1.ProjectStatus.PUBLISHED } }),
                projectRepo.count({ where: { status: ResearchProject_1.ProjectStatus.DRAFT } }),
                projectRepo.count({ where: { status: ResearchProject_1.ProjectStatus.ARCHIVED } }),
                projectRepo.count({ where: { created_at: (0, typeorm_1.Between)(today, now) } }),
                projectRepo.count({ where: { created_at: (0, typeorm_1.Between)(thisWeekStart, now) } }),
                projectRepo.count({ where: { created_at: (0, typeorm_1.Between)(thisMonthStart, now) } })
            ]);
            // Event basic stats
            ...
            const communityChartData = {
                topCommunitiesByProjects: communitiesWithMostProjects.map((c) => ({
                ...
                communityGrowth: communityGrowthOverTime.map((item) => ({
                    month: new Date(item.month).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
                    count: parseInt(item.count)
                })),
                communityTypeDistribution: communityTypeDistribution.map((item) => ({
                    type: item.type,
                    count: parseInt(item.count)
                })),
                communityCategoryDistribution: communityCategoryDistribution.map((item) => ({
                ...
                communitiesByJoinRequests: communitiesByJoinRequests.map((item) => ({
            ...
            const projectChartData = {
                ...
                growthOverTime: projectGrowthOverTime.map((item) => ({
                    month: new Date(item.month).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
                    total: parseInt(item.count),
                    published: parseInt(item.published_count)
                })),
                fieldOfStudyDistribution: fieldOfStudyDistribution.map((item) => ({
                    field: item.field || 'Other',
                    count: parseInt(item.count)
                })),
                collaborationStats: {
                    seekingCollaborators: parseInt((collaborationStats === null || collaborationStats === void 0 ? void 0 : collaborationStats.seeking_collaborators) || 0),
                    collaborative: parseInt((collaborationStats === null || collaborationStats === void 0 ? void 0 : collaborationStats.collaborative) || 0),
                    solo: parseInt((collaborationStats === null || collaborationStats === void 0 ? void 0 : collaborationStats.solo) || 0),
                    totalCollaborators: parseInt((collaborationStats === null || collaborationStats === void 0 ? void 0 : collaborationStats.total_collaborators) || 0)
                },
                ...
                engagement: {
                    totalViews: parseInt(projectEngagement === null || projectEngagement === void 0 ? void 0 : projectEngagement.totalViews) || 0,
                    totalDownloads: parseInt(projectEngagement === null || projectEngagement === void 0 ? void 0 : projectEngagement.totalDownloads) || 0,
                    totalLikes: parseInt(projectEngagement === null || projectEngagement === void 0 ? void 0 : projectEngagement.totalLikes) || 0,
                    totalComments: parseInt(projectEngagement === null || projectEngagement === void 0 ? void 0 : projectEngagement.totalComments) || 0,
                    avgViews: Math.round(parseFloat(projectEngagement === null || projectEngagement === void 0 ? void 0 : projectEngagement.avgViews)) || 0,
                    avgDownloads: Math.round(parseFloat(projectEngagement === null || projectEngagement === void 0 ? void 0 : projectEngagement.avgDownloads)) || 0
                },
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const [usersCount, projectsCount, communitiesCount, eventsCount] = await Promise.all([
                    userRepo.count({ where: { date_joined: (0, typeorm_1.Between)(date, nextDate) } }),
                    projectRepo.count({ where: { created_at: (0, typeorm_1.Between)(date, nextDate) } }),
                    communityRepo.count({ where: { created_at: (0, typeorm_1.Between)(date, nextDate) } }),
                    eventRepo.count({ where: { created_at: (0, typeorm_1.Between)(date, nextDate) } })
                ]);
            ...
            const summary = {
                ...
                users: {
                    ...
                    growthRate: newUsersLastMonth > 0
                        ...
                        : 0,
                    byAccountType: usersByType.reduce((acc, item) => {
                ...
                communities: {
                    ...
                    engagement: {
                        totalMembers: parseInt(communityEngagement === null || communityEngagement === void 0 ? void 0 : communityEngagement.totalMembers) || 0,
                        totalPosts: parseInt(communityEngagement === null || communityEngagement === void 0 ? void 0 : communityEngagement.totalPosts) || 0,
                        avgMembersPerCommunity: Math.round(parseFloat(communityEngagement === null || communityEngagement === void 0 ? void 0 : communityEngagement.avgMembersPerCommunity) || 0),
                        pendingJoinRequests
                    },
                ...
                events: {
                    total: totalEvents,
                    upcoming: await eventRepo.count({ where: { status: Event_1.EventStatus.UPCOMING } }),
                    ongoing: await eventRepo.count({ where: { status: Event_1.EventStatus.ONGOING } }),
                    completed: await eventRepo.count({ where: { status: Event_1.EventStatus.COMPLETED } }),
                    cancelled: await eventRepo.count({ where: { status: Event_1.EventStatus.CANCELLED } })
                },
                content: {
                    blogs: {
                        total: await blogRepo.count(),
                        published: await blogRepo.count({ where: { status: BlogPost_1.BlogStatus.PUBLISHED } })
                    },
                    qa: {
