...
const CollaborationEmailTemplates_1 = require("../helpers/CollaborationEmailTemplates");
...
class CollaborationController {
     ...
     */
    static async requestCollaboration(req, res) {
        try {
            ...
            if (project.collaboration_status === ResearchProject_1.CollaborationStatus.SOLO) {
                return res.status(400).json({
                    success: false,
            ...
            const existingRequest = await requestRepo.findOne({
                where: {
                    ...
                    requester: { id: userId },
                    status: CollaborationRequest_1.CollaborationRequestStatus.PENDING
                }
            });
            if (existingRequest) {
                return res.status(400).json({
            ...
            const collaborationRequest = requestRepo.create({
                ...
                requester: { id: userId },
                reason: reason.trim(),
                expertise: (expertise === null || expertise === void 0 ? void 0 : expertise.trim()) || null,
                status: CollaborationRequest_1.CollaborationRequestStatus.PENDING
            });
            ...
            const newCollaborationInfo = {
                ...
                status: ResearchProject_1.CollaborationInfoStatus.PENDING,
            ...
            try {
                const emailHtml = CollaborationEmailTemplates_1.CollaborationEmailTemplates.getCollaborationRequestTemplate(project, requester, savedRequest);
     ...
     */
    static async getProjectCollaborationRequests(req, res) {
        try {
            const { id: projectId } = req.params;
            const userId = req.user.userId;
     ...
     */
    static async approveCollaborationRequest(req, res) {
        try {
            ...
            if (request.status !== CollaborationRequest_1.CollaborationRequestStatus.PENDING) {
                return res.status(400).json({
                    success: false,
            ...
            request.status = CollaborationRequest_1.CollaborationRequestStatus.APPROVED;
            ...
            if (project.collaboration_status === ResearchProject_1.CollaborationStatus.SEEKING_COLLABORATORS) {
                project.collaboration_status = ResearchProject_1.CollaborationStatus.COLLABORATIVE;
            }
            // ==================== ENHANCEMENT: UPDATE COLLABORATION_INFO ====================
            console.log("\n📊 Updating collaboration_info...");
            const collaborationInfo = project.collaboration_info || [];
            // Find and update the user's collaboration info
            const userCollabIndex = collaborationInfo.findIndex(info => info.user_id === request.requester.id);
            if (userCollabIndex !== -1) {
                // Update existing entry
                collaborationInfo[userCollabIndex] = {
                    ...collaborationInfo[userCollabIndex],
                    status: ResearchProject_1.CollaborationInfoStatus.APPROVED,
                    updated_at: new Date()
                };
            ...
            console.log("\n📧 ========== EMAIL NOTIFICATION START ==========");
            try {
                const emailHtml = CollaborationEmailTemplates_1.CollaborationEmailTemplates.getRequestApprovedTemplate(request.project, request.requester);
                await (0, utils_1.sendEmail)({
     ...
     */
    static async rejectCollaborationRequest(req, res) {
        try {
            ...
            // Update request status
            request.status = CollaborationRequest_1.CollaborationRequestStatus.REJECTED;
            request.rejection_reason = (rejection_reason === null || rejection_reason === void 0 ? void 0 : rejection_reason.trim()) || null;
            request.responded_at = new Date();
            await requestRepo.save(request);
            ...
            if (userCollabIndex !== -1) {
                ...
                collaborationInfo[userCollabIndex] = {
                    ...collaborationInfo[userCollabIndex],
                    status: ResearchProject_1.CollaborationInfoStatus.REJECTED,
                    updated_at: new Date(),
                    rejection_reason: (rejection_reason === null || rejection_reason === void 0 ? void 0 : rejection_reason.trim()) || null
                };
                console.log("✅ Updated existing collaboration_info entry");
            }
            else {
            ...
            console.log("\n📧 ========== EMAIL NOTIFICATION START ==========");
            try {
                const emailHtml = CollaborationEmailTemplates_1.CollaborationEmailTemplates.getRequestRejectedTemplate(request.project, request.requester, request.rejection_reason);
                await (0, utils_1.sendEmail)({
                    to: request.requester.email,
     ...
     */
    static async getMyCollaborationRequests(req, res) {
        try {
            const userId = req.user.userId;
            const { status } = req.query;
            const requestRepo = db_1.default.getRepository(CollaborationRequest_1.CollaborationRequest);
     ...
     */
    static async getProjectsUserCanContributeTo(req, res) {
        try {
            const userId = req.user.userId;
            const projectRepo = db_1.default.getRepository(ResearchProject_1.ResearchProject);
    ...
    static async addContribution(req, res) {
        try {
            ...
            if (req.files && req.files.contribution_files) {
                const files = Array.isArray(req.files.contribution_files)
                    ? req.files.contribution_files
                    : [req.files.contribution_files];
                ...
                for (const file of files) {
                    try {
                        ...
                        const tempFilePath = path.join(tempDir, `${Date.now()}_${Math.random().toString(36).substring(2, 9)}_${file.originalname}`);
                        ...
                        const uploadResult = await (0, cloud_1.UploadToCloud)(fileWithPath);
            ...
            const contribution = contributionRepo.create({
                ...
                contribution_section: (contribution_section === null || contribution_section === void 0 ? void 0 : contribution_section.trim()) || null,
    ...
    static async updateContribution(req, res) {
        try {
            ...
            if (contribution_title)
                contribution.contribution_title = contribution_title.trim();
            if (contribution_content)
                contribution.contribution_content = contribution_content.trim();
            if (contribution_section !== undefined)
                contribution.contribution_section = (contribution_section === null || contribution_section === void 0 ? void 0 : contribution_section.trim()) || null;
                ...
                return res.json({
                    ...
                    data: {
                        author: {
                            ...
                            institution: (_a = project.author.profile) === null || _a === void 0 ? void 0 : _a.institution_name,
                            role: 'Creator'
                        },
                        collaborators: [],
                        total: 0
            ...
            res.json({
                ...
                data: {
                    author: {
                        ...
                        email: project.author.email,
                        profile_picture_url: project.author.profile_picture_url,
                        institution: (_b = project.author.profile) === null || _b === void 0 ? void 0 : _b.institution_name,
                        role: 'Creator'
                    },
    ...
    static async removeCollaborator(req, res) {
        try {
            ...
            console.log("\n📊 Updating collaboration_info...");
            const collaborationInfo = project.collaboration_info || [];
            const updatedCollaborationInfo = collaborationInfo.filter(info => info.user_id !== collaboratorId);
    ...
    static async approveContribution(req, res) {
        try {
            ...
            console.log("\n📧 ========== EMAIL NOTIFICATION START ==========");
            try {
                const emailHtml = CollaborationEmailTemplates_1.CollaborationEmailTemplates.getContributionApprovedTemplate(contribution.project, contribution.contributor, contribution);
                await (0, utils_1.sendEmail)({
