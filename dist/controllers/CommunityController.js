...
class CommunityController {
    ...
    static async searchCommunities(req, res) {
        try {
            ...
            const { q = "", page = 1, limit = 10, category, community_type, sort_by = "relevance" } = req.query;
            ...
            // Apply search query
            if (q && q !== "") {
                queryBuilder.andWhere("(community.name ILIKE :q OR community.description ILIKE :q)", { q: `%${q}%` });
            }
            // Apply filters
            if (category) {
    ...
    static async deleteCommunity(req, res) {
        try {
            ...
            await db_1.default.transaction(async (transactionalEntityManager) => {
                ...
                const postRepo = transactionalEntityManager.getRepository(CommunityPost_1.CommunityPost);
                const joinRequestRepo = transactionalEntityManager.getRepository(CommunityJoinRequest_1.CommunityJoinRequest);
                ...
                console.log("✅ Community found:", {
                    ...
                    eventCount: ((_a = community.events) === null || _a === void 0 ? void 0 : _a.length) || 0,
                    projectCount: ((_b = community.projects) === null || _b === void 0 ? void 0 : _b.length) || 0
                });
                ...
                if (community.posts && community.posts.length > 0) {
                    ...
                    const postsResult = await postRepo
                        .createQueryBuilder()
                        .delete()
                        .from(CommunityPost_1.CommunityPost)
                        .where("community_id = :communityId", { communityId: community.id })
                        .execute();
                ...
                if (community.projects && community.projects.length > 0) {
                    ...
                    await projectRepo
                        .createQueryBuilder()
                        .update(ResearchProject_1.ResearchProject)
                        .set({ community: null })
                        .where("community_id = :communityId", { communityId: community.id })
                        .execute();
                ...
                try {
                    ...
                    const emailHtml = DeleteCommunityTemplate.getDeletionTemplate(communityData, creatorData, reason, adminInfo);
                    await (0, utils_1.sendEmail)({
                        to: creatorData.email,
