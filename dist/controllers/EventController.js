...
class EventController {
    ...
    static async deleteEvent(req, res) {
        try {
            ...
            event.status = "Cancelled"; // Or you can add a "Deleted" status
            await eventRepo.save(event);
            res.json({
                success: true,
    ...
    static async postponeEvent(req, res) {
        try {
            ...
            try {
                // Send to organizer
                const organizerEmail = AdminEventManagementTemplates_1.AdminEventManagementTemplates.getEventPostponedTemplate(event, { oldDates, newDates: { start_datetime: newStart, end_datetime: newEnd }, reason }, 'organizer');
                await (0, utils_1.sendEmail)({
                    to: event.organizer.email,
