...
exports.setupCommunityChatHandlers = void 0;
...
const setupCommunityChatHandlers = (io, socket) => {
    ...
    socket.on("join_community_rooms", async (data, callback) => {
        var _a;
        try {
            const userId = (_a = socket.data.user) === null || _a === void 0 ? void 0 : _a.id;
            if (!userId) {
                return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Not authenticated" });
            }
            const joinedRooms = [];
            ...
            for (const communityId of data.communityIds) {
                ...
                if (!communityUsers.has(userId)) {
                    ...
                    communityUsers.set(userId, {
                        userId,
                        userName: (user === null || user === void 0 ? void 0 : user.first_name) || (user === null || user === void 0 ? void 0 : user.username) || "Unknown",
            ...
            callback === null || callback === void 0 ? void 0 : callback({ success: true, joinedRooms });
        }
        catch (error) {
            console.error("Error joining community rooms:", error);
            callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Failed to join rooms" });
        }
    });
    ...
    socket.on("send_community_message", async (data, callback) => {
        ...
        try {
            ...
            console.log("\n📨 [SEND_MESSAGE] Message send attempt:", {
                ...
                content: ((_b = data.content) === null || _b === void 0 ? void 0 : _b.substring(0, 50)) + "...",
            ...
            if (!community) {
                ...
                return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Community not found" });
            }
            const isMember = community.members.some((member) => member.id === userId);
            if (!isMember) {
                console.error("❌ [SEND_MESSAGE] User not a member:", userId);
                return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Not a member of this community" });
            }
            // Get sender info
            const sender = await userRepository.findOne({ where: { id: userId } });
            if (!sender) {
                console.error("❌ [SEND_MESSAGE] Sender not found:", userId);
                return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "User not found" });
            }
            // Determine chat type
            const chatType = data.chat_type || CommunityChatMessage_1.ChatType.COMMUNITY;
            // Validate direct message requirements
            if (chatType === CommunityChatMessage_1.ChatType.DIRECT) {
                if (!data.recipient_user_id) {
                    console.error("❌ [SEND_MESSAGE] Direct message missing recipient");
                    return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Recipient user ID required for direct messages" });
                }
                ...
                if (!recipientIsMember) {
                    ...
                    return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Recipient is not a member of this community" });
                }
                // Prevent self-messaging
                if (data.recipient_user_id === userId) {
                    console.error("❌ [SEND_MESSAGE] Self-messaging attempt");
                    return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Cannot send direct message to yourself" });
                }
            ...
            // Create new message
            const message = new CommunityChatMessage_1.CommunityChatMessage();
            message.community_id = data.communityId;
            message.sender_id = userId;
            message.content = data.content;
            message.message_type = data.messageType || CommunityChatMessage_1.MessageType.TEXT;
            ...
            if (chatType === CommunityChatMessage_1.ChatType.DIRECT && data.recipient_user_id) {
                ...
                console.log("👤 [SEND_MESSAGE] Recipient user fetched:", {
                    recipientId: recipientUser === null || recipientUser === void 0 ? void 0 : recipientUser.id,
                    recipientName: (recipientUser === null || recipientUser === void 0 ? void 0 : recipientUser.first_name) || (recipientUser === null || recipientUser === void 0 ? void 0 : recipientUser.username)
                });
            }
            ...
            if (chatType === CommunityChatMessage_1.ChatType.COMMUNITY) {
            ...
            else if (chatType === CommunityChatMessage_1.ChatType.DIRECT) {
            ...
            callback === null || callback === void 0 ? void 0 : callback({ success: true, data: responseData });
            console.log("✅ [SEND_MESSAGE] Message send complete\n");
        }
        catch (error) {
            console.error("❌ [SEND_MESSAGE] Error sending message:", error);
            callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Failed to send message" });
        }
    });
    ...
    socket.on("edit_message", async (data, callback) => {
        ...
        try {
            ...
            if (!message) {
                return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Message not found" });
            }
            // Verify user is message owner
            if (message.sender_id !== userId) {
                return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Not authorized to edit this message" });
            }
            // Update message
            ...
            if (message.chat_type === CommunityChatMessage_1.ChatType.COMMUNITY) {
                io.to(`community_${data.communityId}`).emit("message_edited", updateData);
            }
            else if (message.chat_type === CommunityChatMessage_1.ChatType.DIRECT) {
            ...
            callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Failed to edit message" });
        }
    });
    ...
    socket.on("delete_message", async (data, callback) => {
        ...
        try {
            ...
            if (data.deleteType === "for_everyone") {
                ...
                const isAdmin = (community === null || community === void 0 ? void 0 : community.creator.id) === userId;
                if (!isOwner && !isAdmin) {
                    return callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Not authorized to delete this message" });
                }
                ...
                if (message.chat_type === CommunityChatMessage_1.ChatType.COMMUNITY) {
                    ...
                    io.to(`user_${message.recipient_user_id}_community_${data.communityId}`).emit("message_deleted", deleteData);
                }
            }
            else {
            ...
            callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Failed to delete message" });
        }
    });
    ...
    socket.on("react_to_message", async (data, callback) => {
        ...
        try {
            ...
            if (message.chat_type === CommunityChatMessage_1.ChatType.COMMUNITY) {
                ...
                io.to(`user_${message.recipient_user_id}_community_${data.communityId}`).emit("message_reaction_updated", reactionData);
            }
            callback === null || callback === void 0 ? void 0 : callback({ success: true, reactions: message.reactions });
        }
        catch (error) {
            console.error("Error reacting to message:", error);
            callback === null || callback === void 0 ? void 0 : callback({ success: false, error: "Failed to react to message" });
        }
    });
    ...
    socket.on("fetch_online_members", (data, callback) => {
        const communityUsers = activeCommunityUsers.get(data.communityId);
        if (!communityUsers) {
            return callback === null || callback === void 0 ? void 0 : callback({ success: true, onlineUsers: [] });
        }
        ...
        callback === null || callback === void 0 ? void 0 : callback({ success: true, onlineUsers });
    });
    // ==================== HANDLE DISCONNECT ====================
...
exports.setupCommunityChatHandlers = setupCommunityChatHandlers;
