...
static async login(req: Request, res: Response) {
  ...
  try {
    ...
    if (!user.IsForWhichSystem) {
      updates.IsForWhichSystem = SystemType.ONGERA; // Direct assignment - already lowercase
      needsUpdate = true;
    ...
    const ongeraSession = sessionRepo.create({
      user_id: user.id,
      system: SystemType.ONGERA, // Direct assignment - already lowercase
    ...
    const bwengeSession = sessionRepo.create({
      user_id: user.id,
      system: SystemType.BWENGE_PLUS, // Direct assignment - already lowercase
  ...
  try {
    ...
    if (user && !user.social_auth_provider) {
      ...
      user = userRepo.create({
        ...
        IsForWhichSystem: SystemType.ONGERA, // Direct assignment - already lowercase
        institution_ids: [],
        is_institution_member: false,
      });
  ...
  try {
    ...
    await dbConnection.transaction(async (transactionalEntityManager) => {
      ...
      console.log("✅ User found:", {
        ...
        communitiesCount: user.created_communities?.length || 0,
        eventsCount: user.organized_events?.length || 0
      });
