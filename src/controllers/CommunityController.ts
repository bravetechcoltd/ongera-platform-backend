...
static async editCommunity(req: Request, res: Response) {
  try {
    ...
    if (req.file) {
      console.log("🖼️ Uploading new cover image:", req.file.originalname);
      try {
    ...
    console.log("✅ Community updated successfully:", community.id);
    ...
    // ==================== OPTIONAL: NOTIFY MEMBERS ABOUT UPDATE ====================
    if (community.members && community.members.length > 0) {
      try {
        console.log(`📧 Notifying ${community.members.length} members about community update...`);
        
        // You can implement email notification logic here if desired
        // For now, just log it
        console.log(`✅ Member notifications would be sent here`);
        
      } catch (notifyError: any) {
        console.error("❌ Failed to notify members:", notifyError.message);
...
/**
 * Check if user can edit community (helper endpoint)
 * GET /communities/:id/can-edit
 */
static async canEditCommunity(req: Request, res: Response) {
  try {
    CommunityController.logRoute('canEditCommunity', req);
    ...
    console.error("❌ Can edit community error:", error);
    res.status(500).json({ 
      success: false, 
