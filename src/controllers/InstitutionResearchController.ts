...
async function canUserSeeProject(userId: string, project: InstitutionResearchProject): Promise<boolean> {
...
export class InstitutionResearchController {
  ...
  static async listProjects(req: Request, res: Response) {
    try {
      ...
      if (user.account_type === AccountType.INSTITUTION || user.account_type === AccountType.ADMIN) {
        qb.andWhere("institution.id = :instId", { instId: user.id });
      } else if (user.is_industrial_supervisor) {
        qb.andWhere("supervisors.id = :uid", { uid: userId });
      } else {
        // student or instructor
        qb.andWhere("(students.id = :uid OR instructors.id = :uid OR supervisors.id = :uid)", { uid: userId });
      }
  ...
  static async getProject(req: Request, res: Response) {
    try {
      ...
      const allowed = await canUserSeeProject(userId, project);
      if (!allowed) {
  ...
  static async updateProject(req: Request, res: Response) {
    try {
      ...
      if (!isOwner) return res.status(403).json({ success: false, message: "Only an assigned student can edit this project" });
      ...
      // Handle file uploads — new version
      const files = (req.files as any) || {};
      const cover = files.cover_image?.[0];
      const main = files.project_file?.[0];
      const additional = files.additional_files || [];
      ...
      if (main) {
        // bump versions for main file: mark current latest as not latest, add new version
      ...
      if (wasRework) {
        ...
        await logActivity(id, userId, InstitutionActivityType.REWORK_SUBMITTED, `Rework cycle #${project.rework_count} submitted`);

        // Notify supervisors and instructors
        await notifyUsers(
  ...
  static async submitProject(req: Request, res: Response) {
    try {
      ...
      await logActivity(id, userId, InstitutionActivityType.SUBMITTED, `Project submitted for review`);

      // Notify supervisors AND instructors
      const targets = [...(project.industrial_supervisors || []), ...(project.instructors || [])];
      await notifyUsers(
        targets,
        "New Institution Research Project Submitted",
        buildEmail(
          "A new project awaits your review",
  ...
  static async instructorReview(req: Request, res: Response) {
    try {
      ...
      if (project.status !== InstitutionProjectStatus.UNDER_INSTRUCTOR_REVIEW) {
  ...
  static async publishProject(req: Request, res: Response) {
    try {
      ...
      if (!project.institution || project.institution.id !== userId) {
      ...
      const review = reviewRepo.create({
        ...
        reviewer: { id: userId } as any,
        reviewer_role: InstitutionReviewerRole.INSTITUTION_ADMIN,
        action: visibility === "PUBLIC" ? InstitutionReviewAction.PUBLISHED_PUBLIC : InstitutionReviewAction.PUBLISHED_PRIVATE,
        feedback: notes,
        stage: InstitutionReviewStage.INSTITUTION_STAGE,
        is_final: true,
      });
      ...
      await notifyUsers(
        ...
        buildEmail(
          "Project published",
          `Your research project '<b>${project.title}</b>' has been published as <b>${visibility === "PUBLIC" ? "PUBLIC (visible across Bwenge)" : "PRIVATE (institution space only)"}</b>.`
        )
      );
  ...
  static async dashboard(req: Request, res: Response) {
    try {
      ...
      if (user.account_type === AccountType.INSTITUTION) {
        const all = await projectRepo.find({
          where: { institution: { id: user.id } as any },
          relations: ["students", "instructors", "industrial_supervisors"],
        });
        ...
        return res.json({
          ...
          data: {
            ...
            pendingSupervisor: all.filter((p) =>
              [InstitutionProjectStatus.SUBMITTED, InstitutionProjectStatus.UNDER_SUPERVISOR_REVIEW].includes(p.status)
            ).length,
            pendingInstructor: all.filter((p) => p.status === InstitutionProjectStatus.UNDER_INSTRUCTOR_REVIEW).length,
          },
        });
      }
