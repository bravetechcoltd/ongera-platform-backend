...
export class MonthlyStarTrackerController {
   ...
   */
  static async getApprovedStars(req: Request, res: Response) {
    try {
      console.log("\n⭐ ========== GET APPROVED STARS ==========");
      ...
      if (month) {
        queryBuilder.andWhere("tracker.month = :month", { month: parseInt(month as string) });
      }

      if (year) {
        queryBuilder.andWhere("tracker.year = :year", { year: parseInt(year as string) });
      }
